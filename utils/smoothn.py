...
def smoothn(
    ...
    """
     ...
     SMOOTHN用于1-D到N-D数据的稳健样条曲线平滑。
     SMOOTHN为任何维度的数据提供了一种快速、自动化和稳健的离散平滑样条曲线。

     Z=SMOOTHN(Y)自动平滑均匀采样的阵列Y。Y可以是任何N-D噪声阵列(时间序列、图像、3D数据…)。非有限数据(NaN或Inf)被视为缺失值。

     Z=SMOOTHN(Y,S)使用平滑参数S对阵列Y进行平滑。
     S必须是实的正标量。S越大，输出越平滑。如果省略平滑参数S(见前面的选项)或为空(即S=[])，则使用广义交叉验证(GCV)方法自动确定。

     Z=SMOOTHN(Y,W)或Z=SMOTHN(Y,W,S)指定实正值的加权数组W，该数组的大小必须与Y相同。请注意，零权重对应于缺失的值。


     Robust smoothing
     ----------------
     Z = SMOOTHN(...,'robust') 执行鲁棒平滑，以最小化外围数据的影响。

     [Z,S] = SMOOTHN(...) 还返回S的计算值，以便在需要时可以随后微调平滑。

     迭代过程是在存在加权值和/或缺失值的情况下使用的。Z = SMOOTHN(...,OPTION_NAME,OPTION_VALUE)
     使用 OPTION_NAME 和 OPTION_VALUE 指定的终止参数进行平滑。它们可以包含以下标准：
         -----------------
...
def peaks(n):
    ...
    for i in iter(range(int(n / 5 + 0.5))):
        x0 = random.random() * n
        y0 = random.random() * n
